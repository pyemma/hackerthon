...
def getKeyword(text):
    ...
    re = sortedKeywords[0:min(7, math.floor(totalKeywords / 3))]
    result = []
    for tup in re:
        result.append(tup[0])
    return result
...
def main():
    text = """
    (CNN)Andy Murray clocked up 500 wins on tennis' ATP World Tour on Tuesday -- and then celebrated by tucking into a giant cake.
...
"Also to my family, friends and my team for all their encouragement and support and for always sticking by me and to the haters I love you all."
    ...
    print (sortedKeywords[0:min(7, math.floor(totalKeywords / 3))])
