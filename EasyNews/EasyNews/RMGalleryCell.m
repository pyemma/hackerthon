...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self)
    {
        ...
        [_textView setFont:[UIFont fontWithName:@"ArialMT" size:26]];
...
- (void)setImage:(UIImage *)image inSize:(CGSize)imageSize
{
     ...
     */
    _imageView.frame = CGRectMake((self.bounds.size.width - imageSize.width) / 2.0 , 90.0, imageSize.width, imageSize.height);
...
{
    _textView.frame = CGRectMake(15.0, 105.0 + self.image.size.height + 15.0, self.bounds.size.width - 30.0, self.bounds.size.height - (105.0 + self.image.size.height + 15.0 + 15.0));
    _textView.text = text;}
